...
import type { EmbedCoursePayload } from '../courseRegistry';
...
/**
 * Inicializa el panel /admin/embed/ con datos ya serializados desde el servidor (define:vars).
 * No depende de #coursesJson para evitar condiciones de carrera entre scripts.
 */
export function initAdminEmbed(courses: EmbedCoursePayload[]) {
  ...
  const pBackground = $('pBackground') as HTMLInputElement;
    ...
    for (const c of courses) {
      ...
      for (const l of lessons) {
        ...
        <td class="px-2 py-1 align-top font-nunito font-black text-[10px] leading-snug text-textPrimary">
          <span class="line-clamp-2 wrap-break-word">${escapeAttr(c.name)}</span>
