...
export const pooBasicChallenges: PooChallenge[] = [
  ...
  },
  {
    id: 11,
    slug: 'personaje-rpg',
    title: 'Héroe de Videojuego RPG',
    context: 'Un personaje jugable con estadísticas de vida, maná, nivel y estado vivo/muerto en una partida.',
    task: 'Implementa PersonajeRPG. Recibe 40 de daño, usa poción para curar 25 HP, gasta maná para lanzar un hechizo y sube de nivel aumentando sus estadísticas.',
    base: {
      name: 'PersonajeRPG',
      attrs: ['- nombre: String', '- nivel: int', '- puntos_vida: int', '- puntos_mana: int', '- esta_vivo: boolean'],
      methods: ['+ recibir_danio(cantidad: int): void', '+ curar(cantidad: int): void', '+ gastar_mana(costo: int): boolean', '+ subir_nivel(): void'],
    },
    children: [],
  },
  {
    id: 12,
    slug: 'inventario-jugador',
    title: 'Inventario del Jugador (Mochila)',
    context: 'Un inventario de ítems con límite de slots de almacenamiento y peso máximo soportado.',
    task: 'Implementa Inventario. Agrega ítems validando capacidad y peso, usa consumibles, suelta objetos y verifica si queda espacio libre.',
    base: {
      name: 'Inventario',
      attrs: ['- capacidad_maxima: int', '- items_actuales: int', '- peso_maximo_kg: double', '- peso_actual_kg: double'],
      methods: ['+ agregar_item(peso_item: double): boolean', '+ usar_item(peso_item: double): void', '+ soltar_item(peso_item: double): void', '+ tiene_espacio(): boolean'],
    },
    children: [],
  },
  {
    id: 13,
    slug: 'nave-espacial',
    title: 'Nave Espacial de Combate (Arcade)',
    context: 'Una nave espacial en un juego shoot \'em up con escudo deflector, combustible y cañón láser.',
    task: 'Implementa NaveEspacial. Dispara ráfagas láser, recibe impactos en el escudo, activa turbo propulsor y recarga energía.',
    base: {
      name: 'NaveEspacial',
      attrs: ['- modelo: String', '- escudo_energia: int', '- combustible_litros: double', '- municion_laser: int'],
      methods: ['+ disparar(): boolean', '+ recibir_impacto(danio: int): void', '+ activar_turbo(): void', '+ recargar_escudo(cantidad: int): void'],
    },
    children: [],
  },
  {
    id: 14,
    slug: 'jefe-mazmorra',
    title: 'Jefe de Mazmorra (Raid Boss)',
    context: 'Un poderoso jefe de calabozo con múltiples fases de combate, barra de vida y modo furia activado al bajar del 30% de salud.',
    task: 'Implementa JefeMazmorra. Aplica daño al jefe, pasa a la fase 2 al vaciar la primera barra, activa furia para potenciar ataques y reinicia el encuentro si los jugadores caen.',
    base: {
      name: 'JefeMazmorra',
      attrs: ['- nombre_jefe: String', '- vida_maxima: int', '- vida_actual: int', '- fase_combate: int', '- modo_furia: boolean'],
      methods: ['+ recibir_danio(danio: int): void', '+ cambiar_fase(): void', '+ activar_furia(): void', '+ reiniciar_jefe(): void'],
    },
    children: [],
  },
  {
    id: 15,
    slug: 'carta-tcg',
    title: 'Carta Coleccionable de Batalla (TCG)',
    context: 'Una carta coleccionable estilo juego de cartas táctico con coste de energía, poder de ataque, puntos de defensa y rareza dorada.',
    task: 'Implementa CartaTCG. Invoca la carta restando coste, ejecuta ataques contra cartas rivales, aplica bonificadores de daño y recibe ataques enemigos.',
    base: {
      name: 'CartaTCG',
      attrs: ['- nombre_carta: String', '- costo_energia: int', '- puntos_ataque: int', '- puntos_defensa: int', '- es_dorada: boolean'],
      methods: ['+ invocar(): boolean', '+ atacar(objetivo: String): int', '+ potenciar_ataque(bonus: int): void', '+ recibir_ataque(danio: int): void'],
    },
    children: [],
  },
  {
    id: 16,
    slug: 'mando-consola',
    title: 'Mando de Consola Inalámbrico',
    context: 'Un periférico/gamepad inalámbrico con indicador de batería, conexión Bluetooth, motores de vibración háptica y asignación de jugador.',
    task: 'Implementa MandoConsola. Conéctalo a la consola, dispara pulsos de vibración en colisiones, gestiona la batería y apágalo al desconectar.',
    base: {
      name: 'MandoConsola',
      attrs: ['- id_jugador: int', '- bateria_porcentaje: int', '- esta_conectado: boolean', '- vibracion_activa: boolean'],
      methods: ['+ conectar(): void', '+ desconectar(): void', '+ activar_vibracion(duracion_ms: int): void', '+ recargar_bateria(porcentaje: int): void'],
    },
    children: [],
  },
  {
    id: 17,
    slug: 'cofre-loot',
    title: 'Cofre de Recompensas (Loot Chest)',
    context: 'Un cofre de tesoro con recompensas de oro según su rareza, bloqueo por llave y posibilidad de ser un monstruo Mímico.',
    task: 'Implementa CofreLoot. Intenta forzar apertura, desbloquea con la llave correcta, recoge las monedas de oro y activa la trampa si resulta ser mímico.',
    base: {
      name: 'CofreLoot',
      attrs: ['- tipo_rareza: String', '- monedas_oro: int', '- esta_bloqueado: boolean', '- es_mimico: boolean'],
      methods: ['+ desbloquear(tipo_llave: String): boolean', '+ abrir_cofre(): void', '+ reclamar_oro(): int', '+ activar_trampa(): void'],
    },
    children: [],
  },
  {
    id: 18,
    slug: 'auto-carreras',
    title: 'Auto de Carreras (Simulador de Velocidad)',
    context: 'Un auto deportivo de carreras con telemetría de velocidad, marchas secuenciales, tanque de óxido nitroso (NOS) y desgaste de neumáticos.',
    task: 'Implementa AutoCarreras. Acelera en recta, sube marchas secuenciales, activa el turbo de nitro para rebasar y frena dosificando los neumáticos.',
    base: {
      name: 'AutoCarreras',
      attrs: ['- escuderia: String', '- velocidad_kmh: double', '- marcha_actual: int', '- nitro_disponible: int', '- desgaste_neumaticos: double'],
      methods: ['+ acelerar(incremento: double): void', '+ frenar(decremento: double): void', '+ cambiar_marcha(nueva_marcha: int): void', '+ activar_nitro(): void'],
    },
    children: [],
  },
  {
    id: 19,
    slug: 'torre-defensa',
    title: 'Torre de Defensa (Tower Defense)',
    context: 'Una estructura defensiva con radio de alcance, daño base, tipo elemental (Fuego, Hielo, Rayo) y árbol de mejoras.',
    task: 'Implementa TorreDefensa. Ataca oleadas enemigas dentro de su rango, mejora al siguiente nivel para ganar poder, cambia de elemento y gestiona su venta.',
    base: {
      name: 'TorreDefensa',
      attrs: ['- tipo_elemento: String', '- rango_ataque_metros: double', '- danio_base: int', '- nivel_mejora: int', '- costo_mejora: int'],
      methods: ['+ atacar_objetivo(distancia: double): boolean', '+ mejorar_torre(): void', '+ cambiar_elemento(nuevo_elemento: String): void', '+ vender_torre(): int'],
    },
    children: [],
  },
  {
    id: 20,
    slug: 'pase-batalla',
    title: 'Pase de Batalla de Temporada',
    context: 'Sistema de progresión de temporada con niveles (1-100), acumulación de puntos de experiencia (XP), pase Free vs Premium y desbloqueo de skins.',
    task: 'Implementa PaseBatalla. Gana XP en partidas, sube niveles de pase, adquiere la versión Premium y reclama recompensas cosméticas exclusivas.',
    base: {
      name: 'PaseBatalla',
      attrs: ['- temporada_numero: int', '- nivel_actual: int', '- exp_acumulada: int', '- exp_para_siguiente_nivel: int', '- es_premium: boolean'],
      methods: ['+ ganar_experiencia(puntos_exp: int): void', '+ subir_nivel_pase(): void', '+ comprar_premium(): void', '+ reclamar_recompensa(): String'],
    },
    children: [],
  },
];
